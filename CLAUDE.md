...
- **Database creation**: each service has its own MySQL schema (`commodity`, `agent`, `market`, `simulation`, `finance`). `CREATE DATABASE` statements live in `deploy/mysql/init.sql`, which MySQL auto-runs once on first container boot. When adding a new service, append a `CREATE DATABASE IF NOT EXISTS <name>` block to init.sql AND run `make mysql-bootstrap` to re-apply against any existing `mysql_data` volume (the init.sql auto-run only fires on fresh volumes).
...
Some agent/swarm tooling writes per-developer runtime state into the repo
tree. It mutates on every tool run and is **gitignored, not tracked** — do
not `git add` it back:

- `.claude-flow/` (any depth, incl. `web/.claude-flow/`) — claude-flow data,
  neural stats, swarm state.
- `.swarm/state.json` and `.swarm/*.db*` — swarm runtime state.

Project-shared Claude config stays tracked: `.claude/settings.json` (the
plugin/MCP baseline) and the `.claude/skills/` subtree. Per-developer
overrides belong in `.claude/settings.local.json` (already gitignored).

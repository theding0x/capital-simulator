...
- **Store.** `CompositionStore` interface in `store.go`. `memoryComposition` in `memory_composition.go` (in-memory, for tests). `mysql_composition.go` (production). Migrations `00035_v2_ch08_composition.sql` (8 tables) and `00036_v2_ch08_seed.sql`: £10,000 10-year spinning machine (Marx §I), British railway with unequal subcomponents (rails 2yr, sleepers 8yr, earthworks 80yr), auxiliary material (lubricating oil) and raw material (iron ore).
...
| Ch. 8     | ✅ Done     | Fixed Capital and Circulating Capital                                                               | simulation-engine                 |

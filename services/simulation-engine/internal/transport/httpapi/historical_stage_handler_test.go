...
func TestCreateHistoricalStage_Success(t *testing.T) {
	t.Parallel()
	st := store.NewMemory()
	h := httpapi.New(nil, httpapi.Deps{HistoricalStages: st, EnclosureEvents: st})
...
func TestCreateHistoricalStage_BadRequest_MalformedJSON(t *testing.T) {
	t.Parallel()
	h := httpapi.New(nil, httpapi.Deps{HistoricalStages: store.NewMemory(), EnclosureEvents: store.NewMemory()})
